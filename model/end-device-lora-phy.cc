...
EndDeviceLoraPhy::GetTypeId (void)
{
  ...
  .AddTraceSource ("EndDeviceOn",
                   "Trace source indicating the ED is on",
                   MakeTraceSourceAccessor (&EndDeviceLoraPhy::m_isOn),
...
EndDeviceLoraPhy::SwitchToTurnOn (void)
{
  ...
  if (IsEnergyStateOk ())
    // We won't need to enter in KO state, but this updates the energy source
    {
      ...
      NS_LOG_DEBUG ("Actually turning on");
      m_state = TURNON;
...
EndDeviceLoraPhy::UnregisterListener (EndDeviceLoraPhyListener *listener)
{
    ...
    if (!(capacitorEnergySource == 0))
      {
        ...
        if (capacitorEnergySource->IsDepleted ())
          ...
          {
            NS_LOG_DEBUG ("Energy state ok! Actual voltage= " <<
                          capacitorEnergySource->GetActualVoltage());
              return true;
          }
      }
  ...
  EndDeviceLoraPhy::SetCheckForEnergyDepletion (void)
  {
    ...
    if (!(nodeEnergySourceContainer == 0))
      {
        NS_LOG_DEBUG ("found nodeEnergySourceContainer pointer");
      }
    else
      {
        NS_LOG_DEBUG ("Energy Source Container source not found in the node!");
        return;
      }
    Ptr<EnergySource> nodeEnergySource = nodeEnergySourceContainer->Get (0);
    // If CapacitorEnergySource
