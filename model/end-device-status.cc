...
EndDeviceStatus::GetCompleteReplyPacket (void)
{
  ...
  if (m_reply.payload) // If it has APP data to send
  ...
  else // If no APP data needs to be sent, use an empty payload
    {
      ...
      replyPacket = Create<Packet> (m_replyPayloadSize);
    }
  ...
  NS_LOG_DEBUG ("Final size of the reply after adding headers " << replyPacket->GetSize());
...
void
EndDeviceStatus::SetReplyPayloadSize (int replyPayloadSize)
{
  NS_LOG_FUNCTION ("Payload size " << replyPayloadSize);
  m_replyPayloadSize = replyPayloadSize;
}
