                              ...
                              double frequencyMHz, double txPowerDbm)
{
  ...
  // TODO do this only if enough energy
  ...
  NS_LOG_DEBUG("SCHEDULING SWITCH TO STANDBY AFTER " << duration.GetSeconds());
  ...
  if (!m_txFinishedCallback.IsNull ())
    {
      ...
      m_txFinishedEventId = Simulator::Schedule (duration,
...
SimpleEndDeviceLoraPhy::InterruptTx (void)
{
  ...
  NS_LOG_DEBUG("Packet to interrupt: " << m_sendingPacket);
