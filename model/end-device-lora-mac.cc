  ...
  static TypeId tid =
      TypeId ("ns3::EndDeviceLoraMac")
          ...
          .AddTraceSource ("AggregatedDutyCycle",
                           ...
                           MakeTraceSourceAccessor (&EndDeviceLoraMac::m_aggregatedDutyCycle),
                           "ns3::TracedValueCallback::Double")
          .AddAttribute ("DataRate", "Current data rate employed by this device", UintegerValue (0),
          ...
          .AddAttribute ("MaxTransmissions", "Maximum number of transmissions for a packet",
                         IntegerValue (8), MakeIntegerAccessor (&EndDeviceLoraMac::m_maxNumbTx),
          ...
          .AddAttribute ("MType", "Specify type of message will be sent by this ED.",
                         ...
                         MakeEnumChecker (LoraMacHeader::UNCONFIRMED_DATA_UP, "Unconfirmed",
                                          LoraMacHeader::CONFIRMED_DATA_UP, "Confirmed"))
          .AddAttribute ("DeSynchDelay", "Delay to remove synchronization between devices",
                         TimeValue (Seconds (0)),
                         MakeTimeAccessor (&EndDeviceLoraMac::m_deSynchDelay), MakeTimeChecker ())
          .AddAttribute ("FastRx2", "Whether to employ a the same DR in RX1 and RX2",
                         BooleanValue (false), MakeBooleanAccessor (&EndDeviceLoraMac::m_fastRx2),
                         MakeBooleanChecker ())
...
EndDeviceLoraMac::EndDeviceLoraMac ()
    : m_enableDRAdapt (false),
      ...
      // LoraWAN default
      m_receiveWindowDurationInSymbols (12),
...
r // Initialize number of packets sent before changing DR
  ResetEdAdrResilience();
}
...
EndDeviceLoraMac::Send (Ptr<Packet> packet)
{
  ...
  if (!(txChannel && m_retxParams.retxLeft > 0))
    ...
    // the transmitting channel is available and we have not run out the maximum number of retransmissions
    {
      ...
      double ack_timeout = m_uniformRV->GetValue (0, m_deSynchDelay.GetSeconds ());

      m_nextTx =
          Simulator::Schedule (Seconds (ack_timeout), &EndDeviceLoraMac::DoSend, this, packet);
    }
}
...
{
  NS_LOG_FUNCTION (this << packet << m_retxParams.packet);
...
EndDeviceLoraMac::SendToPhy (Ptr<Packet> packetToSend)
{
  ...
  NS_LOG_DEBUG ("Retx left: " << unsigned (m_retxParams.retxLeft));
  NS_LOG_DEBUG ("MaxNumbTx: " << unsigned (m_maxNumbTx));
  NS_LOG_DEBUG ("Actual DR: " << unsigned (m_dataRate));
  ...
  NS_LOG_DEBUG ("Packet duration (s): " << duration.GetSeconds ()
                                        << " Channel bandwidth for this data rate "
                                        << GetBandwidthFromDataRate (m_dataRate));
...
//////////////////////////
void
EndDeviceLoraMac::ResetEdAdrResilience ()
{
  NS_LOG_FUNCTION_NOARGS();
  m_EdAdrResilienceCounter = m_EdAdrResilience + 1; // the first tx will happen with the set DR
}

void
EndDeviceLoraMac::LowerDataRateIfNeeded ()
{
  NS_LOG_FUNCTION_NOARGS ();

  NS_LOG_DEBUG (m_retxParams.waitingAck);
  if (m_enableDRAdapt && m_retxParams.waitingAck)
    {
      m_txPower = 14; // Reset transmission power

      NS_LOG_DEBUG("ResilienceCounter= " << unsigned(m_EdAdrResilienceCounter) <<
                   "Resilience = " << unsigned(m_EdAdrResilience));

      // It's time to decrease the DR
      if (m_EdAdrResilienceCounter % m_EdAdrResilience == 0)
        {
          if ((m_dataRate != 0) && (m_dataRate != 16))
            {
              if (m_dataRate == 6)
                {
                  m_dataRate = 20;
              ...
              NS_LOG_DEBUG ("Adapting DR at the node... new DR: " << unsigned (m_dataRate));

            }
          // Reset counter because DR decreased
          ResetEdAdrResilience();
...
EndDeviceLoraMac::TxFinished (Ptr<const Packet> packet)
{
  ...
  Simulator::Schedule (m_receiveDelay1, &EndDeviceLoraMac::OpenFirstReceiveWindow, this);

  // Schedule the opening of the second receive window
  m_secondReceiveWindow =
      Simulator::Schedule (m_receiveDelay2, &EndDeviceLoraMac::OpenSecondReceiveWindow, this);
...
EndDeviceLoraMac::OpenFirstReceiveWindow (void)
{
  ...
  m_closeFirstWindow = Simulator::Schedule (Seconds (m_receiveWindowDurationInSymbols * tSym),
                                            &EndDeviceLoraMac::CloseFirstReceiveWindow,
...
EndDeviceLoraMac::OpenSecondReceiveWindow (void)
{
  ...
  NS_LOG_DEBUG ("Duration of second receive window in symbols: "
                << unsigned (m_receiveWindowDurationInSymbols));
  NS_LOG_DEBUG (
      "Duration of second receive window in seconds: " << m_receiveWindowDurationInSymbols * tSym);
  // Schedule return to sleep after "at least the time required by the end
  // device's radio transceiver to effectively detect a downlink preamble"
  // (LoraWAN specification)
  ...
  for (it = logicalChannels.begin (); it != logicalChannels.end (); ++it)
    {
      ...
      Time channelWaitingTime = m_channelHelper.GetWaitingTime (logicalChannel);
      waitingTime = std::min (waitingTime, channelWaitingTime);

      NS_LOG_DEBUG ("Waiting time before the next transmission in channel with frequency "
                    << frequency << " is = " << channelWaitingTime.GetSeconds ());
      NS_LOG_DEBUG ("To transmit, we have to wait " << waitingTime.GetSeconds () << " seconds.");
    }
  ...
  if (!m_closeFirstWindow.IsExpired () || !m_closeSecondWindow.IsExpired ())
    {
      ...
      Time endSecondRxWindow = (Time (m_closeSecondWindow.GetTs ()) + MicroSeconds (1));
      NS_LOG_DEBUG (
          "Time at which the second receive window will end: " << endSecondRxWindow.GetSeconds ());
...
void
EndDeviceLoraMac::resetRetransmissionParameters ()
{
  NS_LOG_FUNCTION (this);
...
void
EndDeviceLoraMac::SetEdAdrResilience(uint8_t resilience)
{
  NS_LOG_FUNCTION (this << unsigned(resilience));

  m_EdAdrResilience = resilience;
  ResetEdAdrResilience ();
}
...
void
EndDeviceLoraMac::OnLinkAdrReq (uint8_t dataRate, uint8_t txPower, std::list<int> enabledChannels,
                                int repetitions)
{
  ...
  if (channelMaskOk && dataRateOk && txPowerOk)
    {
      ...
      // Set the data rate
      m_dataRate = dataRate;
      NS_LOG_DEBUG ("After receiving ADR command DR set to " << unsigned(m_dataRate));
...
void
EndDeviceLoraMac::OnNewChannelReq (uint8_t chIndex, double frequency, uint8_t minDataRate,
                                   uint8_t maxDataRate)
{
  NS_LOG_FUNCTION (this);
...
void
EndDeviceLoraMac::SetLogicalChannel (uint8_t chIndex, double frequency, uint8_t minDataRate,
                                     uint8_t maxDataRate)
{
...
void
EndDeviceLoraMac::AddSubBand (double startFrequency, double endFrequency, double dutyCycle,
...
EndDeviceLoraMac::GetFirstReceiveWindowDataRate (void)
{
  NS_LOG_DEBUG ("m_dataRate " << unsigned(m_dataRate) <<
                " m_rx1DrOffset " << unsigned(m_rx1DrOffset) <<
                "it will return " <<
                unsigned(m_replyDataRateMatrix.at(m_dataRate).at(m_rx1DrOffset)));
