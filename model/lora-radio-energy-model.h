...
public:
  ...
  /** * Compute the voltage expenditure of the device when starting from a given
  voltage, staying in a given state for a given duration * Default:
  harvestedPower taken from the capacitor at current time */
  double ComputeLoraVoltageConsumption (EndDeviceLoraPhy::State state,
                                        Time duration, double initialVoltage,
                                        double harvestedPower=-1);
  /**
   * Compute the initial voltage when ending at a given voltage
   */
  double ComputeLoraInitialVoltage (EndDeviceLoraPhy::State state, Time duration,
                                     double finalVoltage, double harvestedPower);

      private : void DoDispose (void);
