...
namespace ns3 {
  ...
  VariableEnergyHarvester::GetTypeId (void)
  {
    static TypeId tid = TypeId ("ns3::VariableEnergyHarvester")
      ...
      .AddAttribute ("PeriodicHarvestedPowerUpdateInterval",
                     ...
                     MakeTimeChecker ())
      .AddAttribute ("Filename", "TODO Add description",
  ...
  void
  VariableEnergyHarvester::AssignPowerTrace(std::vector<double> power)
  {
    NS_LOG_FUNCTION (this);
    m_power = power;
  }
  ...
  void
  VariableEnergyHarvester::UpdatePower (void)
  {
    NS_LOG_FUNCTION (this);

    m_harvestedPower = GetPowerFromSavedTrace (Simulator::Now());
  ...
  double
  VariableEnergyHarvester::GetPowerFromSavedTrace (Time time)
  {
    NS_LOG_FUNCTION (this);

    uint t = std::floor(time.GetSeconds());
    if (m_power.size() > 1)
      {
        // NS_LOG_DEBUG("File size " << m_power.size() );
        NS_ASSERT_MSG(t <= m_power.size (), "t " << t << " power_size: " << m_power.size());
        NS_LOG_DEBUG("t: " << std::to_string(t)
                     <<" s, Power from saved trace is: "
                     << m_power[t] << " W");
        return m_power[t];
      }
    else
  ...
  double
  VariableEnergyHarvester::GetAveragePower (Time time, double samples)
  {
    NS_LOG_FUNCTION (this);

    int t = std::floor(time.GetSeconds());

    double meanP;
    // std::vector<double> averageP (samples, 0);
    double averageP = 0;
    for(int i=0; i<samples; i++)
      {
        if ((t-i) >= 0)
          {
            NS_LOG_DEBUG ("Power at time: " << t - i << " is " << m_power[t - i]);
            averageP += (m_power[t-i]);
          }
        else
          {
            NS_LOG_DEBUG ("Power at time: 0 is " << m_power[0]);
            averageP += (m_power[0]);
          }
      }
    // meanP = std::accumulate (averageP.begin (), averageP.end (), 0) / samples;
    meanP = averageP/samples;
    NS_LOG_DEBUG ("Mean: " << meanP);
    return meanP;
  }
