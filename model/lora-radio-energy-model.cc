...
LoraRadioEnergyModel::HandleEnergyChanged (void)
{
  ...
  if (!m_energyChangedCallback.IsNull () )
    {
      Ptr<EndDeviceLoraPhy> edPhy = m_device->GetPhy ()->GetObject<EndDeviceLoraPhy> ();
      NS_LOG_DEBUG ("Calling the callback if device in sleep mode...");
      if (edPhy->GetState () == EndDeviceLoraPhy::SLEEP)
        {
...
double
LoraRadioEnergyModel::ComputeLoraVoltageConsumption (EndDeviceLoraPhy::State state,
                                                     Time duration,
                                                     double initialVoltage,
                                                     double harvestedPower)
{
  NS_LOG_FUNCTION(this << state << duration << initialVoltage);
  NS_LOG_DEBUG("State: " << state << " duration (s): " << duration.GetSeconds());

  double current = GetCurrentForState(state);
  double voltageConsumption = 0;
  Ptr<CapacitorEnergySource> capacitor = m_source -> GetObject<CapacitorEnergySource>();
  if (!(capacitor == 0))
    {
      NS_LOG_DEBUG("Iload " << current);
      if (harvestedPower == -1) // Default: value not provided
        {
          harvestedPower = capacitor-> GetHarvestersPower();
        }
      voltageConsumption = capacitor->ComputeVoltage(initialVoltage, current,
                                                     harvestedPower, duration);
    }

  return initialVoltage - voltageConsumption;
}

double
LoraRadioEnergyModel::ComputeLoraInitialVoltage (EndDeviceLoraPhy::State state,
                                                 Time duration,
                                                 double finalVoltage,
                                                 double harvestedPower)
{
  NS_LOG_FUNCTION(this << state << duration << finalVoltage);
  NS_LOG_DEBUG("State: " << state << " duration (s): " << duration.GetSeconds());
  ...
  if (!(capacitor == 0))
    {
      ...
      initialVoltage = capacitor->ComputeInitialVoltage (finalVoltage, current,
                                                     harvestedPower, duration);
    }
